...
# from app.web.api.autonomous_agents.schema import AutonomousAgent
from scripts.main import construct_prompt, AutonomousAgent
...
@router.post("/autonomous_agents")
async def create_autonomous_agent(body: Dict):
    ai_config = AIConfig(body["ai_name"], body["ai_role"], body["ai_goals"], interactive_input=False)
    ai_config.save()

    return JSONResponse(
        content={"message": "Autonomous agent created", "id": 1})  # only one main agent is supported for now

@router.patch("/autonomous_agents/{id}")
async def start_autonomous_agent(body: Dict):
    if body["status"] == 'active':  # this is the only status that is supported for now
        ...
        thread = threading.Thread(target=autonomous_agent.start_loop)
        try:
            thread.start()
        except SystemExit:
        ...
        return JSONResponse(content={"message": "Autonomous agent created", "thread_id": thread_id})
        ...
        if thread:
            ...
            if thread.is_alive():  # if a thread cannot be terminated within 30 seconds, it is terminated forcefully.
                ...
                return JSONResponse(content={"message": "Autonomous agent terminated forcefully."})
            else:
                return JSONResponse(content={"message": "Autonomous agent terminated gracefully."})
        else:
